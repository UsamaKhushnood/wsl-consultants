  ...
  methods: {
    getStudent() {
      ...
      if (vm.getUser.type == 'Sales Agent') {
        url = process.env.VUE_APP_API_URL + '/sales-agent/notexpected-leads'
      } else if (vm.getUser.type == 'Call Center Agent') {
        url = process.env.VUE_APP_API_URL + '/call-agent/notexpected-leads'
      } else {
        url = process.env.VUE_APP_API_URL + '/admin/notexpected-leads'
      }
      axios
        .get(url)
        .then((response) => {
